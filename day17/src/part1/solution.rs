    ...
    match to {
        Direction::North => {
            if *from == Direction::North
                || row == 0
                || (*from == Direction::South && straight_line_count >= 3)
            {
                return false;
            }
        }
        Direction::South => {
            if *from == Direction::South
                || row == matrix.len() - 1
                || (*from == Direction::North && straight_line_count >= 3)
            {
                return false;
            }
        }
        Direction::East => {
            if *from == Direction::East
                || col == matrix[0].len() - 1
                || (*from == Direction::West && straight_line_count >= 3)
            {
                return false;
            }
        }
        Direction::West => {
            if *from == Direction::West
                || col == 0
                || (*from == Direction::East && straight_line_count >= 3)
            {
                return false;
            }
        }
...
fn visit(
    ...
    if is_bottom_right(matrix, current_position) {
...
mod tests {
    ...
    fn it_can_check_if_is_able_to_go() {
        ...
        assert_eq!(
            is_able_to_go(&matrix, &((0, 1), Direction::North, 1, 4), Direction::North),
            false
        );
        assert_eq!(
            is_able_to_go(&matrix, &((0, 1), Direction::North, 1, 4), Direction::South),
            true
        );
        assert_eq!(
            is_able_to_go(&matrix, &((0, 1), Direction::North, 1, 4), Direction::East),
            true
        );
        assert_eq!(
            is_able_to_go(&matrix, &((0, 1), Direction::North, 1, 4), Direction::West),
            true
        );

        assert_eq!(
            is_able_to_go(&matrix, &((1, 1), Direction::North, 1, 6), Direction::North),
            false
        );
        assert_eq!(
            is_able_to_go(&matrix, &((1, 1), Direction::North, 1, 6), Direction::South),
            true
        );
        assert_eq!(
            is_able_to_go(&matrix, &((1, 1), Direction::North, 1, 6), Direction::East),
            true
        );
        assert_eq!(
            is_able_to_go(&matrix, &((1, 1), Direction::North, 1, 6), Direction::West),
            true
        );
        ...
        assert_eq!(
            is_able_to_go(&matrix, &((3, 1), Direction::North, 3, 21), Direction::East),
            true
        );
        assert_eq!(
            is_able_to_go(&matrix, &((3, 1), Direction::North, 3, 21), Direction::West),
            true
        );
    }
