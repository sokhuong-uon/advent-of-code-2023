...
fn solution(input: &str) -> u32 {
    ...
    let mut rays: Vec<(Position, Direction)> = Vec::new();
    let max_y = matrix.len() - 1;
    let max_x = matrix[0].len() - 1;

    // From north and south
    for x in 0..=max_x {
        rays.push(((0, x), Direction::North));
        rays.push(((max_y, x), Direction::South));
    }
    // From east and west
    for y in 0..=max_y {
        rays.push(((y, 0), Direction::West));
        rays.push(((y, max_x), Direction::East));
    }
    let mut max = 0;
    rays.iter().for_each(|ray| {
        let mut visited: Map = HashMap::new();

        visit(&matrix, &mut visited, &ray.0, ray.1.clone());
        if visited.len() > max {
            max = visited.len();
        }
    });
